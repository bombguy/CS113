...
public class skill4_button : MonoBehaviour {
    Button skillButton;
    Image skillImage;
    Text skillName;
    ...
    void updateSkill() {
        if (BattleManager.selectedUnit == null)
        {
            ...
            if (current_skill == null)
                addSkilltoButton(BattleManager.selectedUnit.skill4);
            else if (current_skill != BattleManager.selectedUnit.skill4)
                addSkilltoButton(BattleManager.selectedUnit.skill4);
        }
    }
    ...
    void testButton() {
        baseSkill test = new BreakAndContinue();
